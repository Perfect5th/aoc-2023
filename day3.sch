;; Produces a list of n number of n chars
(define fill-n
  (lambda (n)
    (let f ([i 0] [acc '()])
      (if (< i n)
...
;; Converts a string into a vector of containing numbers and symbols.
(define parse-line
  (lambda (str)
    (let f ([i 0] [w 0] [acc '()] [curr '()])
      (if (>= i (string-length str))
          (if (not (null? curr))
              (list->vector (reverse (cons (string->number (list->string (reverse curr))) acc)))
              (list->vector (reverse acc)))
          (let ([char (string-ref str i)])
            (cond
             [(char-numeric? char) (f (+ i 1) (+ w 1) acc (cons char curr))]
             [(not (null? curr))
              (f (+ i 1) 0
                 `(,char ,@(fill-n (- w 1)) ,(string->number (list->string (reverse curr))) ,@acc)
                 '())]
             [else (f (+ i 1) 0 (cons char acc) curr)]))))))

;; Produces a list of ascending integers based on the decimal with of n
(define get-width
  (lambda (n)
    (let f ([n* n] [l 0] [acc '()])
      (if (negative? (log n* 10))
          acc
          (f (/ n* 10) (+ l 1) (cons l acc))))))

;; Produces a list of all the adjacent chars to location i j in sch.
;; Tags gears with their adjacent numbers
(define adjacencies
  (lambda (i j sch)
    (let* ([width (get-width (vector-ref (vector-ref sch i) j))]
           [num-width (length width)]
           [height (vector-length sch)]
           [len (vector-length (vector-ref sch i))]
           [has-up (> i 0)]
           [has-left (> j 0)]
           [has-right (< j (- len num-width))]
           [has-down (< i (- height 1))])
      `(,(and has-up has-left (vector-ref (vector-ref sch (- i 1)) (- j 1)))   ; above-left
        ,@(if has-up (map (lambda (w)
                            (vector-ref (vector-ref sch (- i 1)) (+ j w))) width)
              '())
        ,(and has-up has-right (vector-ref (vector-ref sch (- i 1)) (+ j num-width))) ; above-right
        ,(and has-left (vector-ref (vector-ref sch i) (- j 1)))                 ; left
        ,(and has-right (vector-ref (vector-ref sch i) (+ j num-width)))             ; right
        ,(and has-down has-left (vector-ref (vector-ref sch (+ i 1)) (- j 1))) ; below-left
        ,@(if has-down (map (lambda (w)
                              (vector-ref (vector-ref sch (+ i 1)) (+ j w))) width)
              '())
        ,(and has-right has-down (vector-ref (vector-ref sch (+ i 1)) (+ j num-width)))))))

;; Same as adjacencies, but includes the loc of each item
(define adjacencies2
  (lambda (i j sch)
    (let* ([width (get-width (vector-ref (vector-ref sch i) j))]
           [num-width (length width)]
           ...
           [has-right (< j (- len num-width))]
           [has-down (< i (- height 1))])
      `(,(and has-up has-left `(,(cons (- i 1) (- j 1)) ,(vector-ref (vector-ref sch (- i 1)) (- j 1))))   ; above-left
        ,@(if has-up (map (lambda (w)
                            `(,(cons (- i 1) (+ j w)) ,(vector-ref (vector-ref sch (- i 1)) (+ j w)))) width)
              '())
        ,(and has-up has-right `(,(cons (- i 1) (+ j num-width)) ,(vector-ref (vector-ref sch (- i 1)) (+ j num-width)))) ; above-right
        ,(and has-left `(,(cons i (- j 1)) ,(vector-ref (vector-ref sch i) (- j 1))))                 ; left
        ,(and has-right `(,(cons i (+ j num-width)) ,(vector-ref (vector-ref sch i) (+ j num-width))))             ; right
        ,(and has-down has-left `(,(cons (+ i 1) (- j 1)) ,(vector-ref (vector-ref sch (+ i 1)) (- j 1)))) ; below-left
        ,@(if has-down (map (lambda (w)
                              `(,(cons (+ i 1) (+ j w)) ,(vector-ref (vector-ref sch (+ i 1)) (+ j w)))) width)
              '())
        ,(and has-right has-down `(,(cons (+ i 1) (+ j num-width)) ,(vector-ref (vector-ref sch (+ i 1)) (+ j num-width))))))))

;; True if the location at i and j is adjacent to a non-dot char.
(define valid-part?
  (lambda (i j sch)
    (find (lambda (x)
            (and x
...
;; Produces a list of parts adjacent to non-dot chars in sch
(define valid-parts
  (lambda (sch)
    (let f ([i 0] [j 0] [acc '()])
      (cond
       [(>= i (vector-length sch)) acc]
       [(>= j (vector-length (vector-ref sch i)))
        (f (+ i 1) 0 acc)]
       [else (let ([part (vector-ref (vector-ref sch i) j)])
               (if (and (number? part) (valid-part? i j sch))
                   (f i (+ j 1) (cons part acc))
                   (f i (+ j 1) acc)))]))))

;; Produces a list of gears adjacent to the location at i and j, tagged with their location and numbers
(define gears
  (lambda (i j sch)
    (map (lambda (g) (cons (car g) (vector-ref (vector-ref sch i) j)))
         (filter (lambda (x) (and x (not (number? (cadr x))) (char=? (cadr x) #\*)))
                 (adjacencies2 i j sch)))))

;; Like valid-parts, produces lists of gears for each valid part that has one.
(define valid-parts2
  (lambda (sch)
    (let f ([i 0] [j 0] [acc '()])
      (cond
       ...
       [else (let ([part (vector-ref (vector-ref sch i) j)])
               (if (and (number? part) (valid-part? i j sch))
                   (f i (+ j 1) (cons (gears i j sch) acc))
                   (f i (+ j 1) acc)))]))))

;; Fold that combines the list of nums for gears that are at the same location.
;; A gear here is a pair, ((i . j) . number)
;; acc is a hashtable mapping (i . j) to (list number ...)
(define combine-gears
  (lambda (acc gear)
    (if (null? gear)
        acc
        (let ([loc (caar gear)]
              [num (cdar gear)])
          (hashtable-update! acc loc (lambda (g) (cons num g)) '())
          acc))))

;; Calculates the gear-ratio of a gear by multiplying its adjacent numbers, if there are two of them.
;; Otherwise, returns 0.
(define gear-ratios
  (lambda (acc)
    (let ([results (vector-map (lambda (k)
                                 (let ([numbers (hashtable-ref acc k '())])
                                   (if (= (length numbers) 2)
                                       (apply * 1 numbers)
                                       0)))
                               (hashtable-keys acc))])
...
;; Sums the values of the valid parts in the schematic.
(define solve1
  (lambda (input)
    (apply + 0 (valid-parts (list->vector (map parse-line input))))))

;; Sums the gear ratios of all gears in the schematic.
(define solve2
  (lambda (input)
    (apply + 0 (gear-ratios (fold-left combine-gears
                                       (make-hashtable equal-hash equal?)
                                       (valid-parts2 (list->vector (map parse-line input))))))))

;; A 2D vector containing the numbers, symbols and empty spot (x).
(define EXAMPLE
  '("467..114.."
    "...*......"
    "..35..633."
