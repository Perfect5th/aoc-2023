...
;; finds the first incidence of an integer in a symbol
(define find-first-int
  (lambda (value)
    (letrec ([find ;; recursively process a list of characters
              (lambda (v)
                (if (char-numeric? (car v))
                    (car v)
                    (find (cdr v))))])
      (find value))))

;; sums the first and last integers in calibration-value
(define calculate-calibration-value
  (lambda (value)
    (let ([as-list
           (string->list (if (symbol? value)
                             (symbol->string value)
                             (number->string value)))])
      (string->number
       (string (find-first-int as-list)
               (find-first-int (reverse as-list)))))))

;; iterates over a list of symbols and calculates the calibration value for each.
(define solve1
  (lambda (values)
    (fold-left + 0
               (map calculate-calibration-value values))))
...
;; Checks if str starts with val
(define starts-with?
  (lambda (str val)
    (and (<= (string-length val) (string-length str))
         (string=? val (substring str 0 (string-length val))))))

;; Checks is str starts with anything in SPELLED, returning the corresponding char or false
(define starts-with-num?
  (lambda (str)
    (letrec ([search
              (lambda (ns)
                (cond
                 [(null? ns) #f]
                 [(starts-with? str (caar ns)) (cadar ns)]
                 [else (search (cdr ns))]))])
      (search SPELLED))))

;; Replaces spelled-out substrings with single chars, returning a list of chars
(define replace-substr
  (lambda (str)
    (letrec ([replace
              (lambda (s)
                (let ([starts-with-num (starts-with-num? s)])
                  (cond
                   [(= (string-length s) 0) '()]
                   [starts-with-num
                    (cons starts-with-num
                          (replace (substring s 1 (string-length s))))]
                   [else (cons (string-ref s 0)
                               (replace (substring s 1 (string-length s))))])))])
      (replace str))))

;; Converts a symbol to a list of characters, first converting spelled-out integers to single chars.
(define convert-to-list
  (lambda (sym)
    (if (symbol? sym)
        (replace-substr (symbol->string sym))
        (string->list (number->string sym)))))

;; Similar to above, but uses convert-to-list helper
(define calculate-calibration-value2
  (lambda (value)
    (let ([as-list (convert-to-list value)])
      (string->number
       (string (find-first-int as-list)
               (find-first-int (reverse as-list)))))))
...
(define solve2
  (lambda (values)
    (fold-left + 0
               (map calculate-calibration-value2 values))))
